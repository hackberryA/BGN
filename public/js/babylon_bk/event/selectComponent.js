...
import { setStatus } from "../material.js";
...
const selectedComponentData = Array.from({ length: 8 }, () =>
    Array.from({ length: 8 }, () =>
        Array.from({ length: 8 }, ()=>({ tileNo: 0, component: COMPONENT_TYPE.NONE }))
    )
);
...
const setSelectedObject = (targetObject, remove=false) => {
    if (targetObject) {
        if (targetObject.userData.status === STATUS.PREVIEW || targetObject.userData.status === STATUS.HOVER ) {
            ...
            if (targetObject.userData.component === COMPONENT_TYPE.TERRACE_TILE) {
...
export const initialize = () => {
    ...
    dom.setOnclick("selectTerraceTile", () => changePreviewComponent(COMPONENT_TYPE.TERRACE_TILE))
    dom.setOnclick("selectSinglePillar", () => changePreviewComponent(COMPONENT_TYPE.SINGLE_PILLAR))
    dom.setOnclick("selectDoublePillar", () => changePreviewComponent(COMPONENT_TYPE.DOUBLE_PILLAR))
    dom.setOnclick("selectFountain", () => changePreviewComponent(COMPONENT_TYPE.FOUNTAIN))
    dom.setOnclick("selectStair", () => changePreviewComponent(COMPONENT_TYPE.STAIR))
    dom.setOnclick("selectBridge", () => changePreviewComponent(COMPONENT_TYPE.BRIDGE))
}
...
const mouseMoveHandler = (e) => {
    ...
    if (targetObject && (targetObject.userData.status === STATUS.PREVIEW 
        || targetObject.userData.status === STATUS.HOVER))
    {
...
const clickResetHandler = () => {
    ...
    for (let x=0; x<8; x++) {
        for (let z=0; z<8; z++) {
            const found = three.scene.getObjectByName(`${COMPONENT_TYPE.SINGLE_PILLAR}#${x},0,${z}`);
...
const changePreviewComponent = (component) => {
    ...
    // テラスタイル
    if (component === COMPONENT_TYPE.TERRACE_TILE) {
        for (let x=0; x<8; x++) {
            for (let z=0; z<8; z++) {
                for (let y=1; y<8; y++) {
                    const target = componentData[y][x][z]
                    if (target.component !== COMPONENT_TYPE.NONE || target.tileNo > 0 || x == 7 || z == 7) continue;
                    
                    // 下の階に柱が３つなければスキップ
                    let pillarCount = 0;
                    if (componentData[y-1][x][z].component === COMPONENT_TYPE.SINGLE_PILLAR) pillarCount++;
                    if (componentData[y-1][x][z].component === COMPONENT_TYPE.DOUBLE_PILLAR) pillarCount++;
                    if (componentData[y-1][x+1][z].component === COMPONENT_TYPE.SINGLE_PILLAR) pillarCount++;
                    if (componentData[y-1][x+1][z].component === COMPONENT_TYPE.DOUBLE_PILLAR) pillarCount++;
                    if (componentData[y-1][x][z+1].component === COMPONENT_TYPE.SINGLE_PILLAR) pillarCount++;
                    if (componentData[y-1][x][z+1].component === COMPONENT_TYPE.DOUBLE_PILLAR) pillarCount++;
                    if (componentData[y-1][x+1][z+1].component === COMPONENT_TYPE.SINGLE_PILLAR) pillarCount++;
                    if (componentData[y-1][x+1][z+1].component === COMPONENT_TYPE.DOUBLE_PILLAR) pillarCount++;
        ...
        for (let x=0; x<8; x++) {
            for (let z=0; z<8; z++) {
                for (let y=0; y<8; y++) {
                    ...
                    // 既存はスキップ
                    if (target.component !== COMPONENT_TYPE.NONE) continue;
