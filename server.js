...
import WebSocketBackend from "./WebSocketBackend.js";

// const app: express.Express = express();
// const server: http.Server = http.createServer(app);
// let wss: WebSocket.Server = new WebSocket.WebSocketServer({ server });
const app = express();
const server = http.createServer(app);
let wss = new WebSocket.WebSocketServer({ server });
wss = new WebSocket.WebSocketServer({ port: 8080 });

// バックエンド用 WebSocket操作クラスをインスタンス化
const ws = new WebSocketBackend(wss);
...
// const { default: WebSocketBackend } = require("./WebSocketBackend.ts");
